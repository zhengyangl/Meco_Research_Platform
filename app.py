"""
MEco Research Dashboard — Complete Single-Page Application (LIGHT THEME)
"Nature Is Not Optional."
Based on Jacobs et al. (2025), Biomimetics 2025, 10, 784

Light-theme variant. Run with:
...
# ════════════════════════════════════════════════════════════════
# LIGHT-THEME COLOR REFERENCE  (for maintainers)
# ────────────────────────────────────────────────────────────────
#   Backgrounds
#     #F7F5F1  page background (warm paper)
#     #FFFFFF  card / chart-area surface (floats above page)
#     #FBF9F5  subtle inset panels (counters, inputs)
#   Borders / rules
#     #E5E1DA  standard border / hairline rule
#     #DAD5CC  slightly stronger separator
#   Text (warm, dark → light)
#     #2A2722  primary text / display headings
#     #4A453E  body emphasis
#     #6B665E  subtitles / secondary body
#     #8A847B  muted labels
#     #9A938A  faint labels / axis ticks
#     #B8B0A4  decorative / near-invisible captions
#   Accents (deepened for light bg)
#     #3D7A52  green  — Support / primary brand accent
#     #A8740E  amber  — Replace / Provisioning
#     #2E7CB8  blue   — Support flows / Ecology
#     #5F57BD  violet — Cultural
#     #B05A2E  terracotta — gap / warning highlight
# ════════════════════════════════════════════════════════════════
...
.svc-card {
    ...
    box-shadow: 0 1px 2px rgba(42,39,34,.03);
    transition: border-color .25s, background .25s, box-shadow .25s;
}
.svc-sel-provisioning { background: rgba(168,116,14,.07); border-color: #C49A3E !important; }
.svc-sel-cultural      { background: rgba(95,87,189,.07);  border-color: #8A82D8 !important; }
.svc-sel-regulating    { background: rgba(29,140,105,.07); border-color: #1D9E75 !important; }
.svc-sel-supporting    { background: rgba(61,122,82,.07);  border-color: #3D7A52 !important; }
.svc-icon { font-size: 1.3rem; display: block; margin-bottom: 5px; }
.svc-name { font: 500 .84rem/1.2 'Inter', sans-serif; color: #2A2722; margin-bottom: 3px; }
.svc-desc { font: 400 .7rem/1.4 'Inter', sans-serif; color: #8A847B; }
...
div[data-testid="stCheckbox"] label p {
    ...
    color: #9A938A !important; cursor: pointer;
}
div[data-testid="stCheckbox"] label:hover,
div[data-testid="stCheckbox"] label:hover p { color: #4A453E !important; }
...
.badge-sim {
    background: rgba(168,116,14,0.10);
    border: 1px solid rgba(168,116,14,0.28);
    color: #8A5E0B;
}
.badge-dot {
...
def build_sankey(replace, enhance, support, label_r, label_e, label_s):
    """
    Build and return a Plotly Sankey figure for Section 4 (light theme).
...
# ── Zone C: Particle animation (light theme) ─────────────────
_particle_html = """
<div style="width:100%;height:280px;background:#FFFFFF;position:relative;
            overflow:hidden;border-radius:12px;border:1px solid #E5E1DA;
  ...
  <div style="position:absolute;top:0;left:0;width:100%;height:100%;
    ...
    <div style="width:39%;display:flex;flex-direction:column;
                ...
                border-right:1px solid rgba(42,39,34,0.06);">
      <span style="font-family:Georgia,serif;font-size:22px;font-weight:700;
                   color:rgba(29,140,105,0.95);line-height:1;">39%</span>
      <span style="font-family:Inter,sans-serif;font-size:10px;font-weight:500;
                   color:rgba(29,140,105,0.65);letter-spacing:.12em;
    ...
    <div style="width:3%;display:flex;flex-direction:column;
      ...
      <span style="font-family:Georgia,serif;font-size:13px;font-weight:700;
                   color:rgba(46,124,184,0.95);line-height:1;">3%</span>
      <span style="font-family:Inter,sans-serif;font-size:8px;font-weight:500;
                   color:rgba(46,124,184,0.65);writing-mode:vertical-rl;
...
(function() {
  ...
  const ZONES = [
    { xMin: 0.000, xMax: 0.580, color: '#A8740E', label: 'Replace'  },  // 58%
    { xMin: 0.580, xMax: 0.970, color: '#1D8C69', label: 'Enhance'  },  // 39%
    { xMin: 0.970, xMax: 1.000, color: '#2E7CB8', label: 'Support'  },  //  3%
  ];
  ...
  class Particle {
    ...
    init(initY) {
      ...
      this.opacity  = Math.random() * 0.45 + 0.20;
...
# ── Zone D: Stacked area chart (light theme) ─────────────────
...
_area_fig.add_annotation(
    ...
    showarrow=True, arrowhead=2, arrowcolor="#2E7CB8", arrowwidth=1.2,
    ax=90, ay=-55,
    bgcolor="rgba(255,255,255,0.92)", bordercolor="rgba(46,124,184,0.35)",
...
<div style="display:inline-flex;align-items:center;gap:8px;
            background:rgba(61,122,82,0.07);border:1px solid rgba(61,122,82,0.20);
            border-radius:6px;padding:6px 14px;
            font:300 .72rem/1 'Inter',sans-serif;color:#356B49;margin-bottom:.8rem;">
    💡 &nbsp; Click any category tile to zoom in — then click the breadcrumb bar to return.
</div>
""", unsafe_allow_html=True)

_CAT_PAL2 = {
    "Provisioning": {"cat": "#A8740E", "svc": "rgba(168,116,14,0.42)"},
    "Regulating":   {"cat": "#1D8C69", "svc": "rgba(29,140,105,0.38)"},
    "Supporting":   {"cat": "#3D7A52", "svc": "rgba(61,122,82,0.42)"},
    "Cultural":     {"cat": "#5F57BD", "svc": "rgba(95,87,189,0.40)"},
}
_MISSING_CLR = "#E5E1DA"

_ids2, _lbl2, _par2, _val2, _col2, _hov2 = [], [], [], [], [], []
_ids2.append("root"); _lbl2.append("All Ecosystem Services"); _par2.append("")
_val2.append(0); _col2.append("#F7F5F1"); _hov2.append("")
...
for _, _row in _df2.iterrows():
    ...
    if _empty:
        _htxt = (f"<b>{_row['service']}</b><br>"
                 "<span style='color:#8A847B;'>No bio-inspired research found.</span>")
    else:
        _htxt = (f"<b>{_row['service']}</b><br>Total: {_row['total']:,} papers<br>"
                 f"<span style='color:#A8740E;'>Replace: {_row['replace']:,} ({_row['repl_pct']*100:.0f}%)</span><br>"
                 f"<span style='color:#1D8C69;'>Enhance: {_row['enhance']:,} ({_row['enha_pct']*100:.0f}%)</span><br>"
                 f"<span style='color:#2E7CB8;'>Support: {_row['support']:,} ({_row['supp_pct']*100:.0f}%)</span>")
...
_tree_fig = go.Figure(go.Treemap(
    ...
    marker=dict(colors=_col2, line=dict(width=2, color="#FFFFFF"),
                pad=dict(t=24, l=4, r=4, b=4)),
    textfont=dict(size=11, color="#2A2722", family="Inter, sans-serif"),
    textinfo="label", tiling=dict(packing="squarify", pad=2),
    maxdepth=2, root_color="#F7F5F1",
))
_tree_fig.update_layout(paper_bgcolor="#FFFFFF", height=520,
...
# ── Zone D: Sorted horizontal bar (light theme) ──────────────
st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
...
_bar2.add_trace(go.Bar(name="Replace", y=_df2_sorted["service"], x=_df2_sorted["replace"],
    orientation="h", marker=dict(color="rgba(168,116,14,0.78)", line=dict(width=0)),
...
_bar2.add_trace(go.Bar(name="Enhance", y=_df2_sorted["service"], x=_df2_sorted["enhance"],
    orientation="h", marker=dict(color="rgba(29,140,105,0.68)", line=dict(width=0)),
...
_bar2.add_trace(go.Bar(name="Support", y=_df2_sorted["service"], x=_df2_sorted["support"],
    orientation="h", marker=dict(color="rgba(46,124,184,0.82)", line=dict(width=0)),
...
_bar2.add_annotation(
    ...
    font=dict(size=10, color="#B05A2E", family="Inter, sans-serif"),
    showarrow=False, xanchor="left")
_bar2.update_layout(
    barmode="stack", paper_bgcolor="#FFFFFF", plot_bgcolor="#FBF9F5",
    ...
    xaxis=dict(title=dict(text="Bio-inspired publications (real counts, Jacobs et al. 2025)",
               ...
               tickfont=dict(size=11, color="#8A847B"), tickformat=",",
               gridcolor="#ECE8E1", linecolor="#E5E1DA", zeroline=False),
    yaxis=dict(tickfont=dict(size=10, color="#6B665E", family="Inter, sans-serif"),
               linecolor="#E5E1DA", gridcolor="rgba(0,0,0,0)"),
)
...
# ── Zone B: Network graph (light theme) ──────────────────────
...
_net3.add_annotation(x=-0.70, y=0.84, text="Engineering & Materials",
                     font=dict(size=9, color="rgba(168,116,14,0.65)", family="Inter, sans-serif"),
                     showarrow=False)
...
_net3.add_annotation(x=0.75, y=0.80, text="Ecology & Life Sciences",
                     font=dict(size=9, color="rgba(46,124,184,0.65)", family="Inter, sans-serif"),
                     showarrow=False)
_net3.add_annotation(x=0.0, y=0.0, text="← the gap →",
                     font=dict(size=9, color="rgba(42,39,34,0.30)", family="Inter, sans-serif"),
                     showarrow=False)
...
for (_a, _b, _w) in _EDGES3:
    ...
    _op  = max(0.08, _w * 0.55) if not _is_cross else max(0.06, _w * 0.35)
    ...
    _net3.add_trace(go.Scatter(
        ...
        line=dict(width=_wd, color=f"rgba(42,39,34,{_op:.2f})"),
...
for _par3 in ["Replace", "Enhance", "Support"]:
    ...
    _net3.add_trace(go.Scatter(
        ...
        textfont=dict(size=9, color="#4A453E", family="Inter, sans-serif"),
...
# ── Zone C: RoboBee case study (components.html, light theme) ─
st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
...
# ── Zone D: Framing analysis diverging bar (light theme) ─────
st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
...
# ── Zone C: Global accessibility map (light theme) ───────────
st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
...
_map4 = px.scatter_geo(
    ...
    color_continuous_scale=[
        [0.0, "#A33216"], [0.35, "#C2742F"], [0.65, "#5A9C6E"], [1.0, "#3D7A52"],
    ],
    range_color=[15, 100],
...
_map4.update_geos(
    showcountries=True, countrycolor="rgba(218,213,204,0.9)",
    showcoastlines=True, coastlinecolor="rgba(206,200,189,0.8)",
    showland=True, landcolor="#EFEBE4",
    showocean=True, oceancolor="#F7F5F1",
...
_map4.update_layout(
    ...
    coloraxis_colorbar=dict(
        title=dict(text="Open Access %", font=dict(size=11, color="#8A847B")),
        tickfont=dict(size=10, color="#8A847B"), ticksuffix="%",
...
# ── Zone D: 3% Spotlight cards (components.html, light theme) ─
st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .card { background: #FFFFFF; border: 1px solid #E5E1DA; border-radius: 10px; padding: 1.3rem 1.2rem 1.1rem; border-top: 2px solid rgba(61,122,82,0.5); box-shadow: 0 1px 3px rgba(42,39,34,.04); }
  ...
  .card-meta { flex: 1; }
  .card-title { font-family: 'Playfair Display', serif; font-size: .95rem; font-weight: 700; color: #2A2722; line-height: 1.25; margin-bottom: 3px; }
  .card-service { font-size: .62rem; font-weight: 500; letter-spacing: .14em; text-transform: uppercase; color: #356B49; }
  .card-body { font-size: .75rem; font-weight: 300; line-height: 1.72; color: #6B665E; margin-bottom: .85rem; }
  .card-body em { color: #356B49; font-style: italic; }
  .card-footer { display: flex; justify-content: space-between; align-items: center; padding-top: .7rem; border-top: 1px solid #E5E1DA; }
  .card-n { font-family: 'Playfair Display', serif; font-size: 1.3rem; font-weight: 700; color: #2E7CB8; }
  .card-n-label { font-size: .62rem; font-weight: 300; color: #8A847B; margin-top: 1px; }
  .card-paradigm { font-size: .62rem; font-weight: 500; letter-spacing: .12em; text-transform: uppercase; padding: 3px 9px; border-radius: 20px; background: rgba(46,124,184,0.10); color: #246592; border: 1px solid rgba(46,124,184,0.22); }
</style></head><body>
<div class="grid">
  <div class="card">
    <div class="card-top">
...
# ── Zone D: MEco pathway cards (components.html, light theme) ─
