...
_disc_df = papers_df[["wos_categories_parsed", "category"]].copy()
...
# ------Log-Ratio Scoring-----------
# The meaning of log_ratio:
# - Positive score: The higher the score, the more exclusive the word is to Replace.
# - Negative score: The lower the score, the more exclusive the word is to Support.
# - Score close to 0: The word frequently appears in both camps and lacks distinctiveness.
...
def top_each_side(stats, n):
    """
    Extracts the top 'n' terms most strongly associated with each paradigm 
    based on their log-ratio scores.
    
    Args:
        stats (list of dict): The list of term statistics, where each dict must 
                              contain a 'log_ratio' key.
        n (int): The number of extreme terms to extract for each side.
        
    Returns:
        tuple: (replace_leaning, support_leaning).
               replace_leaning is sorted from strongest positive to weakest.
               support_leaning is sorted from strongest negative to weakest.
    """
    # Sort the list from the most negative log-ratio (Support) 
    # to the most positive log-ratio (Replace).
    sorted_by_ratio = sorted(stats, key=lambda x: x["log_ratio"])
    
    # The lowest 'n' scores are the most distinctively Support-leaning terms.
    support_leaning = sorted_by_ratio[:n]              # most negative
    
    # The highest 'n' scores are the Replace-leaning terms.
    # [-n:] grabs the last 'n' items, and [::-1] reverses them so the 
    # absolute strongest Replace terms appear at the very beginning of the list.
...
# ════════════════════════════════════════════════════════════════
# hand-picked 15 terms that are (a) narratively legible to a general reader and (b) representative
# of the two subcorpora's vocabulary. Pure chemical/material abbreviations
# (SiC, ZnO, AgNPs, ROS, MOF, ZIF...) were excluded even where their
# log-ratio was higher, because they carry no meaning without a
# materials-science background.
#
# FINDING WORTH NOTING: the Support-leaning vocabulary is NOT ecological
# language. It's biomimetic-surface and fluid-dynamics language (drag,
# shark skin, aerodynamic, topography) with only two genuinely
# ecological terms (sustainability, ecological) making the cut at all.
# This reinforces §3's core finding from a different angle: even the 3%
# of papers that lean Support don't talk like ecologists — they talk
# like engineers who imitated a biological surface.
...
for _w in FRAMING_WORDS:
    ...
    if _disp in _disc_lookup:
        ...
        # Special case: merge "reactive oxygen" + "oxygen species" —
        # near-identical counts (337 vs 334), average them.
